...
export function deriveOptionsSentiment(analysis, spot) {
  ...
  reasons.push({
    tone: pcrSentiment === "bullish" ? "positive" : pcrSentiment === "bearish" ? "negative" : "neutral",
...
export function deriveSignals({ valuation, quality, risk, options, analystUpsidePct }) {
  ...
  if (valuation.hasData && quality.hasData && risk.hasData && valuation.score >= 65 && quality.score >= 60 && risk.score >= 50) {
    signals.push({
      tone: "positive",
  ...
  if (options.score != null && valuation.hasData && options.score >= 60 && analystUpsidePct >= 0.15) {
    signals.push({
      tone: "positive",
...
export function deriveOpportunity(valuation, quality, risk, options, analystUpsidePct) {
  const upsideScore =
    Number.isFinite(analystUpsidePct) ? Math.max(0, Math.min(100, 50 + analystUpsidePct * 200)) : null;
  const scoreParts = [
    valuation.score,
    quality.score,
    risk.score,
