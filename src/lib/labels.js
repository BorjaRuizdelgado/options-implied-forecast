...
export function distributionLabels(analysis) {
  ...
  return [
    { label: "10th pct", value: fmt(p[10] || 0), color: "#b05040", tooltip: "10% chance price is at or below this level by expiry." },
    { label: "25th pct", value: fmt(p[25] || 0), color: "#c08050", tooltip: "25% chance price is at or below this level by expiry." },
    { label: "50th pct", value: fmt(p[50] || 0), color: "#4d6a61", tooltip: "Median — equal chance price ends above or below this level by expiry." },
    { label: "75th pct", value: fmt(p[75] || 0), color: "#4d6a61", tooltip: "75% chance price is at or below this level by expiry." },
    { label: "90th pct", value: fmt(p[90] || 0), color: "#3d7a5a", tooltip: "90% chance price is at or below this level by expiry." },
  ];
}
...
export function entryLabels(analysis) {
  ...
  const biasColour =
    entry.bias === "bullish" ? "#3d7a5a" :
    entry.bias === "bearish" ? "#b05040" :
    "#1c1c1c";
  const rr = !isNaN(entry.riskReward) ? `${entry.riskReward.toFixed(1)}\u00d7` : "N/A";
  const pcrVol = !isNaN(pcr.pcrVol) ? pcr.pcrVol.toFixed(2) : "N/A";

  return [
    { label: "Bias", value: capitalize(entry.bias), color: biasColour, tooltip: "Directional lean derived from S/R positioning and the options probability distribution." },
    { label: "Spot", value: fmt(analysis.spot), color: "#1c1c1c", tooltip: "Current market price of the asset." },
    { label: "Entry", value: fmt(entry.entry), color: "#c08050", tooltip: "Suggested entry price based on the nearest support or resistance level." },
    { label: "Stop", value: fmt(entry.stop), color: "#b05040", tooltip: "Suggested stop-loss level to cap downside if the trade goes against you." },
    { label: "Target", value: fmt(entry.target), color: "#3d7a5a", tooltip: "Suggested profit target based on the opposing S/R level." },
    ...
    { label: "Sentiment", value: capitalize(pcr.sentiment), color: "#555555", tooltip: "Market sentiment implied by the Put/Call ratio." },
  ];
}
