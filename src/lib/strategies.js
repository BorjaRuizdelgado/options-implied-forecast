...
export function suggestStrategies(analysis, research) {
  ...
  // Compute average IV
  const ivValues = analysis.ivData
    .filter((d) => d.iv > 0)
    .map((d) => d.iv);
  ...
  if (avgIV > 0.30 && Math.abs(skew) < 0.15) {
  ...
  if (skew > -0.15 && valScore >= 45) {
    ...
    if (cspPut) {
      strategies.push({
        ...
        legs: [
          { type: "Put", strike: cspPut.strike, action: "Sell" },
        ],
  ...
  if (skew > 0.05 || (research?.risk?.score != null && research.risk.score >= 60)) {
    const protPut = findNearDeltaStrike(puts, spot, 0.20, "put");
    if (protPut) {
      strategies.push({
        name: "Protective Put",
        description: "Buy an OTM put to hedge downside risk on an existing position.",
        legs: [
          { type: "Put", strike: protPut.strike, action: "Buy" },
        ],
  ...
  if (avgIV < 0.35 && (research?.quality?.score ?? 0) >= 55) {
    const ccCall = findNearDeltaStrike(calls, spot, 0.30, "call");
    if (ccCall) {
      strategies.push({
        name: "Covered Call",
        description: "Sell an OTM call against existing shares to generate income.",
        legs: [
          { type: "Call", strike: ccCall.strike, action: "Sell" },
        ],
